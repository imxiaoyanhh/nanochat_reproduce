...
# Eager mode: torch.compile + mixed dtypes (bfloat16 params, float32 CPU scalars) triggers
# strict lerp_ dtype checks in dynamo. Disable compile to avoid per-op dtype fixes.
...
def adamw_step_fused(
    ...
    # Use .item() to avoid CPU/CUDA device mismatch in newer PyTorch
    p.mul_(1 - (lr_t * wd_t).item())
    # Update running averages (lerp_ is cleaner and fuses well)
    beta1_val = (1 - beta1_t).item()
    beta2_val = (1 - beta2_t).item()
    exp_avg.lerp_(grad, beta1_val)
    exp_avg_sq.lerp_(grad.square(), beta2_val)
    # Bias corrections
    bias1 = (1 - beta1_t ** step_t).item()
    bias2 = (1 - beta2_t ** step_t).item()
    eps_val = eps_t.item()
    denom = (exp_avg_sq / bias2).sqrt() + eps_val
    step_size = (lr_t / bias1).item()
...
# Eager mode: same reason as adamw_step_fused (mixed dtypes under dynamo).
# @torch.compile(dynamic=False, fullgraph=True)
def muon_step_fused(
    ...
    # Nesterov momentum (use .item() to avoid CPU/CUDA device mismatch)
    ...
    # Variance reduction (use .item() to avoid CPU/CUDA device mismatch)
    beta2_val = 1 - beta2_t.item()
    v_mean = g.float().square().mean(dim=red_dim, keepdim=True)
    red_dim_size = g.size(red_dim)
    v_norm_sq = v_mean.sum(dim=(-2, -1), keepdim=True) * red_dim_size
    v_norm = v_norm_sq.sqrt()
    second_momentum_buffer.lerp_(v_mean.to(dtype=second_momentum_buffer.dtype), beta2_val)
    ...
    # Cautious weight decay + parameter update (use .item() to avoid CPU/CUDA device mismatch)
    lr = lr_t.item()
    wd = wd_t.item()
...
class DistMuonAdamW(torch.optim.Optimizer):
    ...
    def _reduce_adamw(self, group: dict, world_size: int) -> dict:
        ...
        for p in group['params']:
            ...
            # Ensure grad is on the same device as the parameter (fixes CPU tensor issues)
